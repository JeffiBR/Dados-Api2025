# main.py (completo e atualizado com análise gráfica) - VERSÃO 3.5.0
import os
import asyncio
...
class CollectionRequest(BaseModel):
    ...
    # ✅ CORREÇÃO: Usar 'days' se 'dias_pesquisa' não fornecido
...
# MODELOS PARA NOTIFICAÇÕES - VERSÃO CORRIGIDA (SEM updated_at)
class NotificationCreate(BaseModel):
    title: str = Field(..., max_length=255)
    message: str
    user_ids: Optional[List[str]] = Field(None, description="IDs específicos de usuários (vazio = todos)")
...
# NOVOS MODELOS PARA ANÁLISE GRÁFICA
...
#----Dados------
@app.get("/analise-dados")
async def serve_analise_dados():
    """Serve a página de análise de dados"""
    with open("web/analise_dados.html", "r", encoding="utf-8") as f:
        return Response(content=f.read(), media_type="text/html")

# Endpoint para upload de avatar usando Supabase Storage
...
async def upload_avatar(
    ...
    try:
        ...
        try:
            # Usar o cliente Supabase admin para ter permissões
            bucket_name = "avatars"
            ...
            try:
                ...
                logging.warning(f"Erro ao verificar/criar bucket: {bucket_error}")
            ...
            # Obter URL pública
            public_url_response = supabase_admin.storage.from_(bucket_name).get_public_url(file_path)
            avatar_url = public_url_response

            logging.info(f"Avatar upload realizado com sucesso: {avatar_url}")

            return {"avatarUrl": avatar_url}

        except Exception as storage_error:
            logging.error(f"Erro no Supabase Storage: {storage_error}")
            raise HTTPException(status_code=500, detail="Erro no serviço de armazenamento")

    except HTTPException:
        raise
    except Exception as e:
        logging.error(f"Erro crítico no upload de avatar: {e}")
        raise HTTPException(status_code=500, detail=f"Erro interno no upload de avatar: {str(e)}")
...
# --- ENDPOINTS DE NOTIFICAÇÕES COMPLETAMENTE CORRIGIDOS (SEM updated_at) ---
...
async def get_user_notifications(
    ...
    """Busca notificações do usuário logado - VERSÃO CORRIGIDA"""
    try:
        start_index = (page - 1) * limit
        end_index = start_index + limit - 1

        # Query para buscar notificações do usuário OU globais (user_id = null)
        query = supabase_admin.table('notifications').select('*', count='exact')

        # Filtro por usuário: notificações globais (sem user_id) ou específicas do usuário
        ...
        notifications_with_creators = []
        for notification in response.data:
            creator_name = None
            if notification.get('created_by'):
                try:
                    creator_resp = await asyncio.to_thread(
                    ...
                    if creator_resp.data:
                        creator_name = creator_resp.data.get('full_name')
                except Exception:
            ...
            notifications_with_creators.append(notification_data)

        return {
            "notifications": notifications_with_creators,
...
async def get_unread_count(current_user: UserProfile = Depends(get_current_user)):
    """Conta notificações não lidas do usuário - VERSÃO CORRIGIDA"""
    try:
        response = await asyncio.to_thread(
        ...
        logging.error(f"Erro ao contar notificações não lidas: {e}")
        raise HTTPException(status_code=500, detail="Erro ao contar notificações não lidas")
...
async def update_notification(
    ...
    """Marca uma notificação como lida - VERSÃO CORRIGIDA SEM updated_at"""
    try:
        print(f"🔔 DEBUG: Tentando marcar notificação {notification_id} como lida para usuário {current_user.id}")

        # Verificar se a notificação existe e pertence ao usuário ou é global
        notif_response = await asyncio.to_thread(
            supabase_admin.table('notifications')
            .select('*')
        ...
        if not notif_response.data:
            print(f"❌ DEBUG: Notificação {notification_id} não encontrada para usuário {current_user.id}")
            raise HTTPException(status_code=404, detail="Notificação não encontrada")

        print(f"✅ DEBUG: Notificação encontrada: ID={notif_response.data[0]['id']}, UserID={notif_response.data[0].get('user_id')}, IsRead={notif_response.data[0].get('is_read')}")

        # Preparar dados para atualização - APENAS is_read, SEM updated_at
        update_data = {
            'is_read': notification_data.is_read
        }

        print(f"✅ DEBUG: Dados de atualização (SEM updated_at): {update_data}")

        # Atualizar a notificação específica
        response = await asyncio.to_thread(
            supabase_admin.table('notifications')
            .update(update_data)
            .eq('id', notification_id)
        ...
        # Verificar erro de forma compatível com a biblioteca Supabase
        if hasattr(response, 'error') and response.error:
            error_msg = getattr(response.error, 'message', str(response.error))
            print(f"❌ DEBUG: Erro ao atualizar notificação: {error_msg}")
            raise HTTPException(status_code=400, detail=f"Erro ao atualizar notificação: {error_msg}")

        if not response.data:
            print(f"❌ DEBUG: Nenhum dado retornado ao atualizar notificação {notification_id}")
            raise HTTPException(status_code=404, detail="Notificação não encontrada após atualização")

        print(f"✅ DEBUG: Notificação {notification_id} atualizada com sucesso: {response.data[0]}")
        ...
        print(f"💥 DEBUG: Erro crítico ao atualizar notificação: {str(e)}")
        logging.error(f"Erro ao atualizar notificação: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Erro interno ao atualizar notificação: {str(e)}")

@app.put("/api/notifications/mark-all-read")
async def mark_all_notifications_read(current_user: UserProfile = Depends(get_current_user)):
    """Marca todas as notificações do usuário como lidas - VERSÃO CORRIGIDA SEM updated_at"""
    try:
        print(f"🔔 DEBUG: Marcando todas as notificações como lidas para usuário {current_user.id}")

        # Dados de atualização - APENAS is_read, SEM updated_at
        update_data = {
            'is_read': True
        }

        # Atualizar notificações do usuário específico e globais não lidas
        response = await asyncio.to_thread(
            supabase_admin.table('notifications')
            .update(update_data)
            .or_(f"user_id.eq.{current_user.id},user_id.is.null")
        ...
        # Verificar erro de forma compatível
        if hasattr(response, 'error') and response.error:
            error_msg = getattr(response.error, 'message', str(response.error))
            print(f"❌ DEBUG: Erro ao marcar todas como lidas: {error_msg}")
            raise HTTPException(status_code=400, detail=f"Erro ao marcar notificações como lidas: {error_msg}")

        updated_count = len(response.data) if response.data else 0
        print(f"✅ DEBUG: {updated_count} notificações marcadas como lidas")

        return {
            "message": f"Notificações marcadas como lidas: {updated_count}", 
            "updated_count": updated_count
        }

    except Exception as e:
        print(f"💥 DEBUG: Erro crítico ao marcar todas como lidas: {str(e)}")
        logging.error(f"Erro ao marcar notificações como lidas: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Erro interno ao marcar notificações como lidas")

@app.post("/api/notifications")
async def create_notification(
    notification_data: NotificationCreate,
    current_user: UserProfile = Depends(require_page_access('notifications'))
):
    """Cria uma nova notificação - VERSÃO CORRIGIDA"""
    try:
        logging.info(f"Admin {current_user.id} criando notificação: {notification_data.title}")
        ...
        if notification_data.user_ids:
            ...
            # Notificação para todos os usuários (global)
            notification = {
                'title': notification_data.title,
                'message': notification_data.message,
                'created_by': current_user.id,
                'is_read': False,
                'created_at': datetime.now().isoformat()
                # user_id é null para notificações globais
            }
            notifications_to_create.append(notification)

        # Inserir notificações
        response = await asyncio.to_thread(
            supabase_admin.table('notifications').insert(notifications_to_create).execute
        )

        if hasattr(response, 'error') and response.error:
            error_msg = getattr(response.error, 'message', str(response.error))
            raise HTTPException(status_code=400, detail=f"Erro ao criar notificação: {error_msg}")

        created_count = len(response.data) if response.data else 0

        logging.info(f"Notificação criada com sucesso para {created_count} destinatários")
        return {
...
# --- ENDPOINT ALTERNATIVO CORRIGIDO ---
...
async def mark_notification_read(
    ...
    """Endpoint alternativo para marcar notificação como lida - MÉTODO POST"""
    try:
        print(f"🔔 ALT: Marcando notificação {notification_id} como lida via POST")

        # Usar o mesmo endpoint de update mas com dados fixos
        notification_data = NotificationUpdate(is_read=True)
        return await update_notification(notification_id, notification_data, current_user)

    except Exception as e:
        print(f"💥 ALT: Erro no endpoint alternativo: {str(e)}")
        raise HTTPException(status_code=500, detail="Erro ao marcar notificação como lida")

# --- ENDPOINTS ADMIN PARA NOTIFICAÇÕES CORRIGIDOS ---
...
async def get_all_notifications_admin(
    ...
    """Busca todas as notificações com paginação (apenas admin) - VERSÃO CORRIGIDA"""
    try:
        if current_user.role != 'admin':
            raise HTTPException(status_code=403, detail="Apenas administradores gerais podem ver todas as notificações")
        ...
        if search:
            query = query.or_(f"title.ilike.%{search}%,message.ilike.%{search}%")
        ...
        logging.error(f"Erro ao buscar todas as notificações: {e}")
        raise HTTPException(status_code=500, detail=f"Erro ao buscar notificações: {str(e)}")
...
async def delete_notification_admin(
    ...
    """Exclui uma notificação e todas as suas instâncias (apenas admin) - VERSÃO CORRIGIDA"""
    try:
        if current_user.role != 'admin':
            raise HTTPException(status_code=403, detail="Apenas administradores gerais podem excluir notificações")

        # Deletar todas as notificações com este ID
        response = await asyncio.to_thread(
        ...
        return {
            "message": f"Notificação e {deleted_count - 1 if deleted_count > 1 else 0} instâncias excluídas",
            "deleted_count": deleted_count
        }

    except Exception as e:
        logging.error(f"Erro ao excluir notificação: {e}")
        raise HTTPException(status_code=500, detail=f"Erro ao excluir notificação: {str(e)}")
...
# --------------------------------------------------------------------------
# --- NOVOS ENDPOINTS PARA ANÁLISE GRÁFICA ---
# --------------------------------------------------------------------------

@app.get("/api/analysis/available-dates")
async def get_available_dates(user: UserProfile = Depends(get_current_user)):
    """Obtém as datas disponíveis para análise baseadas nas coletas"""
    try:
        response = await asyncio.to_thread(
        ...
        dates = list(set([collection['iniciada_em'][:10] for collection in response.data]))
        return sorted(dates)
...
@app.post("/api/price-analysis", response_model=PriceAnalysisResponse)
async def price_analysis(
    request: PriceAnalysisRequest,
    user: UserProfile = Depends(require_page_access('compare'))
):
    """Realiza análise de preços para gráficos"""
    try:
        # Buscar dados do produto no período
        query = supabase.table('produtos').select('*')
        ...
        if request.product_identifier.isdigit() and len(request.product_identifier) > 7:
            ...
            query = query.ilike('nome_produto_normalizado', f"%{request.product_identifier.lower()}%")

        query = query.in_('cnpj_supermercado', request.cnpjs)
        query = query.gte('data_ultima_venda', request.start_date)
        query = query.lte('data_ultima_venda', request.end_date)

        response = await asyncio.to_thread(query.execute)

        if not response.data:
            return PriceAnalysisResponse(bar_chart_data=[], line_chart_data=[])

        # Processar dados para gráfico de barras
        bar_data = await process_bar_chart_data(response.data)

        # Processar dados para gráfico de linha
        line_data = await process_line_chart_data(response.data, request.cnpjs)
        ...
        logging.error(f"Erro na análise de preços: {e}")
        raise HTTPException(status_code=500, detail="Erro ao processar análise")

async def process_bar_chart_data(products_data):
    """Processa dados para gráfico de barras (preço médio por mercado)"""
    try:
        # Agrupar por mercado
        market_prices = {}

        for product in products_data:
            market_name = product['nome_supermercado']
            price = product['preco_produto']

            if price and price > 0:
                if market_name not in market_prices:
                    market_prices[market_name] = []
                market_prices[market_name].append(price)

        # Calcular média por mercado
        bar_data = []
        ...
        logging.error(f"Erro ao processar dados de barras: {e}")
        return []

async def process_line_chart_data(products_data, cnpjs):
    """Processa dados para gráfico de linha (evolução temporal)"""
    try:
        # Buscar nomes dos mercados
        ...
        market_names = {m['cnpj']: m['nome'] for m in markets_response.data}

        # Agrupar por mercado e data
        market_time_series = {}

        for product in products_data:
            market_cnpj = product['cnpj_supermercado']
            market_name = market_names.get(market_cnpj, market_cnpj)
            price = product['preco_produto']
            sale_date = product['data_ultima_venda']

            if price and price > 0 and sale_date:
                # Extrair apenas a data (YYYY-MM-DD)
                if isinstance(sale_date, str):
                    sale_date = sale_date[:10]
                elif hasattr(sale_date, 'date'):
                    sale_date = sale_date.isoformat()[:10]

                key = f"{market_name}_{sale_date}"

                if key not in market_time_series:
                    market_time_series[key] = {
                        'market': market_name,
                        'date': sale_date,
                        'prices': []
                    }

                market_time_series[key]['prices'].append(price)

        # Calcular média diária por mercado
        daily_data = {}
        for key, data in market_time_series.items():
            market = data['market']
            date = data['date']
            avg_price = sum(data['prices']) / len(data['prices'])

            if market not in daily_data:
                daily_data[market] = []

            daily_data[market].append({
                'date': date,
                'price': round(avg_price, 2)
            })

        # Ordenar por data e calcular variação
        line_data = []
        for market, points in daily_data.items():
            ...
            if len(points) > 1:
                for i in range(len(points)):
                    if i == 0:
                        points[i]['variation'] = 0
                    else:
                        prev_price = points[i-1]['price']
                        current_price = points[i]['price']
                        if prev_price > 0:
                            variation = ((current_price - prev_price) / prev_price) * 100
                            points[i]['variation'] = round(variation, 1)
                        else:
                            points[i]['variation'] = 0
        ...
        logging.error(f"Erro ao processar dados de linha: {e}")
        return []
...
@app.get("/")
def read_root():
    return {"message": "Bem-vindo à API de Preços Em Foco - Versão 3.5.0"}