document.addEventListener('DOMContentLoaded', () => {
    ...
    const showMessage = (msg) => {
        resultsGrid.innerHTML = `<div class="empty-state"><h3>${msg}</h3><p>Tente ajustar os termos da busca ou filtros</p></div>`;
    };
    ...
    const performSearch = async (isRealtime = false) => {
        ...
        try {
            ...
            if (currentResults.length === 0) {
                ...
                applyFilters();
                showNotification(`Encontramos ${currentResults.length} resultado(s)`);
            }
        } catch (error) {
    ...
    clearSearchButton.addEventListener('click', () => { searchInput.value = ''; resultsGrid.innerHTML = ''; resultsFilters.style.display = 'none'; searchInput.focus(); });
