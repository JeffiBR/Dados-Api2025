document.addEventListener('DOMContentLoaded', () => {
        ...
        if (data.status === 'RUNNING') {
            ...
            if ((data.status === 'COMPLETED' || data.status === 'FAILED') && data.report) {
                ...
                if (data.status === 'COMPLETED') {
                    showSuccess("Coleta concluída", `Coleta de dados finalizada com sucesso. ${data.report.totalItemsSaved} itens salvos.`, 5000);
                } else {
                    showError("Coleta falhou", "A coleta de dados encontrou problemas e foi interrompida.", 5000);
                }
            } else {
                 reportContainer.style.display = 'none';
            }
    ...
    const checkStatus = async () => {
        try {
            ...
            showError("Erro de status", "Não foi possível verificar o status da coleta.", 3000);
    ...
    const startCollection = async () => {
        ...
        try {
            ...
            showSuccess("Coleta iniciada", data.message, 3000);
            checkStatus();
        } catch (error) {
            showError("Erro ao iniciar", `Falha ao iniciar a coleta: ${error.message}`, 5000);
            checkStatus();
        }
    };
