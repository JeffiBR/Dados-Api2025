document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadSupermarkets = async () => {
        try {
            ...
            showError("Erro ao carregar", "Não foi possível carregar os filtros de mercados.", 5000);
    ...
    const comparePrices = async () => {
        ...
        if (!productIdentifier) { 
            showWarning("Produto necessário", "Por favor, informe um produto.", 3000);
            return; 
        }
        if (selectedCnpjs.length < 2) { 
            showWarning("Múltiplos mercados", "Selecione pelo menos dois mercados para comparar.", 3000);
            return; 
        }
        
        const session = await getSession();
        if (!session) {
            showError("Login necessário", "Você precisa estar logado para usar esta função.", 3000);
            return;
        }
        ...
        try {
            ...
            const data = await response.json();
            renderChart(data);
            showSuccess("Comparação realizada", "Dados de preços carregados com sucesso.", 3000);
        } catch (error) {
            showError("Erro ao buscar", `Erro ao buscar dados: ${error.message}`, 5000);
        } finally {
