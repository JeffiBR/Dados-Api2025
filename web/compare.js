document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupEventListeners() {
        ...
        document.addEventListener('progressCancel', handleSearchCancel);
    }

    function handleSearchCancel() {
        isSearchCancelled = true;
        showNotification('Busca cancelada pelo usuário', 'warning');
    ...
    async function searchCurrentPrices() {
        ...
        progressManager.start(barcodes.length, selectedCnpjs.length);
        ...
        try {
            ...
            // Buscar por códigos de barras com acompanhamento de progresso
            for (let i = 0; i < barcodes.length; i++) {
                ...
                // Atualizar progresso - produto atual
                progressManager.productStart(i, barcodes.length, barcode);

                const productData = await fetchProductPrices(barcode, selectedCnpjs, session, i, barcodes.length);
                if (productData && productData.prices.length > 0) {
                    currentResults.push(productData);
                }

                // Marcar produto como completo
                progressManager.productComplete(i, barcodes.length);
            }

            // Verificar se a busca foi cancelada
            if (isSearchCancelled) {
                showNotification('Busca cancelada', 'warning');
                return;
            }
    ...
    async function fetchProductPrices(barcode, cnpjs, session, productIndex, totalProducts) {
        try {
            const requestBody = { 
                produto: barcode, 
                cnpjs: cnpjs 
            ...
            // Buscar informações dos mercados para exibir nomes
            const marketNames = {};
            allMarkets.forEach(market => {
                marketNames[market.cnpj] = market.nome;
            });

            // Atualizar progresso para cada mercado
            for (let i = 0; i < cnpjs.length; i++) {
                ...
                const cnpj = cnpjs[i];
                const marketName = marketNames[cnpj] || cnpj;
                progressManager.marketStart(marketName, i, cnpjs.length);

                // Pequeno delay para demonstração do progresso (opcional)
                await new Promise(resolve => setTimeout(resolve, 50));

                progressManager.marketComplete(marketName);
            }
