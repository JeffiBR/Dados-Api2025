document.addEventListener('DOMContentLoaded', () => {
    ...
    const performPrune = async () => {
        ...
        if (collection_ids.length === 0) {
            showWarning("Nenhuma coleta selecionada", "Por favor, selecione pelo menos uma coleta para apagar.", 3000);
            return;
        }
        ...
        try {
            ...
            showSuccess("Limpeza realizada", `${data.deleted_count} registros foram apagados com sucesso.`, 3000);
            setTimeout(() => loadCollectionsForMarket(selectedCnpj), 2000);

        } catch (error) {
            resultMessage.textContent = `Erro: ${error.message}`;
            resultMessage.style.color = 'red';
            showError("Erro na limpeza", `Não foi possível realizar a limpeza: ${error.message}`, 5000);
        } finally {
            pruneButton.disabled = false;
