document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadCollections = async () => {
        try {
            ...
            showError("Erro ao carregar", `Não foi possível carregar as coletas: ${error.message}`, 5000);
    ...
    tableBody.addEventListener('click', async (e) => {
        ...
        if (target.classList.contains('details-btn')) {
            ...
            if (!isVisible && detailsContent.innerHTML === 'Carregando detalhes...') {
                 try {
                    ...
                    showError("Erro ao carregar detalhes", error.message, 5000);
                }
            }
        }
        ...
        if (target.classList.contains('delete-btn')) {
            const id = target.dataset.id;
            const confirmed = confirm('Tem certeza que deseja excluir esta coleta? Esta ação não pode ser desfeita.');
            ...
            if (confirmed) {
                try {
                    ...
                    showSuccess("Coleta excluída", "A coleta foi excluída com sucesso.", 3000);
                } catch (error) {
                    console.error('Erro ao excluir coleta:', error);
                    showError("Erro ao excluir", `Não foi possível excluir a coleta: ${error.message}`, 5000);
                }
            }
        }
    });
